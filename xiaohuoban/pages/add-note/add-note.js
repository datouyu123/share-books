...
Page({
    ...
    return s;
            ...
            if (!result.committed) {
              ...
              //  console.log('transaction commit success!');
               //添加note记录到note表
              that.noteRef.child(that.data.proKey).child('notes').push(that.data.note)          
           .then(function(newRef){
             //修改付款人支出数额
            //  console.log(that.data.note.payer.key);
  ...
  onShow:function(){
    ...
    if (value) {
      ...
      // console.log('addnote show data:',that.data);
    }
    } catch (e) {
